...
def get_predicted_value(patient_symptoms):
    ...
    return diseases_list.get(predicted_index, "Disease not found")

def predict_symptoms(new_symptoms):
    symptoms = []
    for symptom in new_symptoms:
        new_symptom_tfidf = loaded_vectorizer.transform([symptom])
        predicted_label = loaded_model.predict(new_symptom_tfidf)[0]
        symptoms.append(predicted_label)
    return symptoms
...
def predict():
    if request.method == 'POST':
        input_symptom = request.form.get('symptoms')
        split_symptoms = [s.strip() for s in input_symptom.split('and')]
        symptoms = predict_symptoms(split_symptoms)
        print(symptoms)
        ...
        medications_list = med[0].strip("[]").split(", ")  # Convert medications string to list

        return render_template('index.html', predicted_disease=predicted_disease, dis_desc=desc, dis_pre=my_precautions,
                               dis_med=medications_list, dis_wrkout=wrkout)
